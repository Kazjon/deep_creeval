...
PATH_HEAD = "tsne_coords_"
PATH_TAIL = ".png"

def save_coords(coords, i, save_path, c):
	plt.figure(figsize=(10,10))
	axes = [-30,30,-30,30]
	plt.axis(axes)
	plt.scatter(coords[:,0],coords[:,1], s=15, c=c, cmap="copper")
	count = "%05d" % i
	plt.savefig(os.path.join(save_path,PATH_HEAD+count+PATH_TAIL), bbox_inches='tight')
	plt.close("all")
...
def _gradient_descent(objective, p0, it, n_iter, n_iter_without_progress=30,
					  ...
					  min_grad_norm=1e-5, min_error_diff=1e-5, verbose=0,
					  args=[],tsne=None, save_path="data", c=None):
	...
	for i in range(it, n_iter):
		save_coords(p.reshape(p.shape[0]/tsne.n_components, tsne.n_components), i, save_path, c)
...
class TSNE(BaseEstimator):
	...
	def _tsne(self, P, alpha, n_samples, random_state, X_embedded=None, c=None):
		"""Runs t-SNE."""
		...
		params, error, it = _gradient_descent(
			...
			args=[P, alpha, n_samples, self.n_components],tsne=self, save_path=self.save_path, c=c)
		params, error, it = _gradient_descent(
		...
		params, error, it = _gradient_descent(
			...
			verbose=self.verbose, args=[P, alpha, n_samples,
										self.n_components],tsne=self, save_path=self.save_path, c=c)
		if self.verbose:
	...
	def tsne_update(self, P_prime, c=None):
		alpha = max(self.n_components - 1.0, 1)
		n_samples = self.embedding_.shape[0]

		params = self.embedding_.ravel()
		# Final optimization
		params, error, it = _gradient_descent(
			_kl_divergence, params, it=self.n_iter, n_iter=self.n_iter+100,
			momentum=0.8, learning_rate=self.learning_rate,
			verbose=self.verbose, args=[P_prime, alpha, n_samples,
										self.n_components],tsne=self, save_path=self.save_path, c=c)
		if self.verbose:
			print("[t-SNE] Error after %d iterations: %f" % (it + 1, error))
		...
		subprocess.call("ffmpeg -b 5000 -f image2 -r 8 -i "+self.save_path+PATH_HEAD+"%05d"+PATH_TAIL+" -vcodec libx264 -y "+self.save_path+PATH_HEAD+".mp4",shell=True)

		return X_embedded



	def fit_transform(self, X, y=None, c=None):
